...
- **対象プロセス以外でも介入しません。** 既定は Warp (`warp.exe`) のみです。日本語入力の
  「じゃ/じゅ/じょ」等に余計な遅延と誤爆を持ち込まないためです。対象を増やしたいときは
  `shift_ime.ahk` の `JK_TARGET_PROCESSES` に exe 名を足してください。
...
- Warpは画面上でnvimとシェルを区別できません（ウィンドウタイトルがカレントディレクトリのため）。
  そのためWarpのシェルプロンプトで日本語入力中に `jk` と打った場合も Esc + IME OFF になります。
  日本語のローマ字入力で `jk` が並ぶことはまずないので実害は出ない想定です。
- IMEのON/OFF状態は、Warpのように IMM32 非対応（TSFのみ）なアプリでは読み出せないため、
  このスクリプト自身が最後に設定した値（Shiftキー操作の結果）を覚えて使っています。
  Warpの外で `半角/全角` キー等を使ってIMEを切り替えると状態がズレることがありますが、
  左右Shiftをどちらか一度叩けば同期し直ります。
